...
const EditBook=(props)=>{
    ...
    useEffect(()=>{
       ...
       // axios.get(`http://localhost:3004/books/${params.kitapId}`)
       // .then(res=>{
       ...
       setShowModal(true)
        ...
        const EditBook=()=>{
            if(bookname===""|| author==="" || category===""){
                alert("ktap adı,kitap yazarı,kategori boş brakılamaz")
                return;
            }
            const updatedBook={
                id:params.kitapId,
            ...
            axios.put(`http://localhost:3004/books/${params.kitapId}`,updatedBook)
            .then(res=>{
                console.log(res)
    ...
    onChange={(event)=>setCategory(event.target.value)}>
        ...
        </div>
        {
            showModal===true && (
                ...
                aciklama={`${bookname} Kitabını güncellemek İçin Onaylayın`}
